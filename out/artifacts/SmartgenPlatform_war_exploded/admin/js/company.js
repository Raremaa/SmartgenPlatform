...
$("#btn_Query").click(function() {
	url = "/SmartgenPlatform/companyGet";
...
$("#btn_Add").click(function() {
	$("#fm_company").form("reset"); //用clear会删除之前form表单的预设格式，用reset则不会
	$("#Identity_no").attr("checked",true);
	$("#dlg_company").dialog("open");
	url = "/SmartgenPlatform/companyAdd";
});

//单击修改按钮
$("#btn_Edit").click(function() {
	$("#fm_company").form("clear");
	var row = $('#dg_company').datagrid('getSelected'); //获取选中行的记录号
	if (row == null) {
		alert("请选择需要修改的记录");
		return;
	}
/*	$.getJSON(row, function(data){
	...
	$("#dlg_company").dialog("open");
	$("#fm_company").form("load", row); //根据选中行的记录号读取该行记录
	url = "/SmartgenPlatform/companyUpdate";
});

//单击保存按钮
function saveRecord() {
		$("#fm_company").form("submit", {
			url : url,
...
$("#btn_Del").click(function() {
	...
	url = "/SmartgenPlatform/companyDelete";
...
//格式化显示
function formatterCompanyidentity(value) {
	switch (value) {
	case 0:
